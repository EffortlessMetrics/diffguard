//! Red tests for work-05d48a76: redundant match arm removal in string_syntax()
//!
//! Issue #418 reports a redundant match arm in `preprocess.rs` where `Language::Json`
//! is explicitly matched but then shadowed by a wildcard `_` pattern that produces
//! the same result (`StringSyntax::CStyle`).
//!
...
//!
//! NOTE: This is a cosmetic/clarity change. The behavioral tests below PASS both before
//! and after the fix because `Language::Json.string_syntax()` returns `StringSyntax::CStyle`
//! regardless of whether it's handled by the explicit arm or the wildcard. There is no
//! behavioral difference to test - this is purely a code clarity improvement.
//!
//! The tests document:
//! - Language::Json should return StringSyntax::CStyle (via wildcard after fix)
...
fn json_included_in_cstyle_group_via_wildcard() {
    ...
    // These should be via explicit arm (YAML, TOML - JSON is redundant and should be removed)
    let explicit_cstyle_yaml_toml = vec![Language::Yaml, Language::Toml];

    // These were previously explicit but should now be via wildcard
    let now_wildcard_json = vec![Language::Json];
    ...
    // Verify YAML and TOML have explicit arms (NOT redundant)
    for lang in explicit_cstyle_yaml_toml {
        assert_eq!(
            lang.string_syntax(),
            StringSyntax::CStyle,
            "{:?} should return CStyle (explicit arm, not redundant)",
            lang
        );
    }

    // Verify JSON is now via wildcard (was previously explicit but redundant)
    for lang in now_wildcard_json {
        assert_eq!(
            lang.string_syntax(),
            StringSyntax::CStyle,
            "{:?} should return CStyle (via wildcard after fix)",
            lang
        );
    }
}
