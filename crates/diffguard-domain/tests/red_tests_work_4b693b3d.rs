...
use diffguard_domain::{
    compile_rules, evaluate_lines, evaluate_lines_with_overrides_and_language, InputLine,
    Language, RuleHitStat,
};
...
fn test_prepare_lines_function_exists_with_correct_signature() {
    ...
    let (prepared_lines, by_file) = diffguard_domain::evaluate::prepare_lines(
        input_lines,
        None,
    );
    ...
    assert!(by_file.contains_key("test.py"), "by_file should contain test.py");
    assert_eq!(by_file.get("test.py").unwrap().len(), 2, "test.py should have 2 lines");
}
...
fn test_prepare_lines_with_forced_language() {
    ...
    let (prepared_lines, _by_file) = diffguard_domain::evaluate::prepare_lines(
        input_lines,
        Some(Language::Rust),
    );
...
fn test_collect_match_events_function_exists_with_correct_signature() {
    ...
    // ) -> Vec<MatchEvent>
    let events = diffguard_domain::evaluate::collect_match_events(
        &by_file,
        &prepared_lines,
        &rules,
        None,
...
fn test_emit_findings_function_exists_with_correct_signature() {
    ...
    let (findings, counts, truncated, rule_hits) = diffguard_domain::evaluate::emit_findings(
        events,
        &rules,
        &prepared_lines,
        100,
...
fn test_helpers_produce_same_results_as_orchestrator() {
    ...
    let orchestrator_result = evaluate_lines_with_overrides_and_language(
        input_lines.clone(),
        &rules,
        100,
        None,
    ...
    let (helper_findings, helper_counts, helper_truncated, helper_rule_hits) =
        diffguard_domain::evaluate::emit_findings(
            events,
            &rules,
            &prepared_lines,
            100,
    ...
    assert_eq!(
        orchestrator_result.counts.warn,
        helper_counts.warn,
        "Warn count should match"
    );
    assert_eq!(
        orchestrator_result.counts.info,
        helper_counts.info,
...
fn test_orchestrator_function_compiles_without_too_many_lines_warning() {
    ...
    let result = evaluate_lines_with_overrides_and_language(
        input_lines,
        &rules,
        100,
        None,
...
fn test_helper_signatures_match_specification() {
    ...
    let events: Vec<_> = diffguard_domain::evaluate::collect_match_events(
        &by_file,
        &prepared_lines,
        &rules,
        None,
    ...
    let (findings, counts, truncated, rule_hits): (Vec<Finding>, VerdictCounts, u32, BTreeMap<String, RuleHitStat>) =
        diffguard_domain::evaluate::emit_findings(
            events,
            &rules,
            &prepared_lines,
