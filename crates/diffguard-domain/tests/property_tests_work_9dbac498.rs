...
// Feature: sanitize-line-refactor, Property Tests for sanitize_line invariants (work-9dbac498)
proptest! {
    #![proptest_config(ProptestConfig::with_cases(100))]

    // Property: Output length is preserved regardless of options (comments_only)
    #[test]
    fn property_sanitize_line_length_preserved_comments_only(
        line in prop::string::string_regex("[ -~]{0,100}").unwrap(),
    ) {
        let mut p = diffguard_domain::Preprocessor::with_language(
            diffguard_domain::PreprocessOptions::comments_only(),
            diffguard_domain::Language::Rust,
        );
        let result = p.sanitize_line(&line);
        prop_assert_eq!(result.len(), line.len());
    }

    // Property: Output length is preserved regardless of options (strings_only)
    #[test]
    fn property_sanitize_line_length_preserved_strings_only(
        line in prop::string::string_regex("[ -~]{0,100}").unwrap(),
    ) {
        let mut p = diffguard_domain::Preprocessor::with_language(
            diffguard_domain::PreprocessOptions::strings_only(),
            diffguard_domain::Language::Rust,
        );
        let result = p.sanitize_line(&line);
        prop_assert_eq!(result.len(), line.len());
    ...
    // Property: Output length is preserved regardless of options (both masks)
    #[test]
    fn property_sanitize_line_length_preserved_both_masks(
        line in prop::string::string_regex("[ -~]{0,100}").unwrap(),
    ) {
        let mut p = diffguard_domain::Preprocessor::with_language(
            diffguard_domain::PreprocessOptions::comments_and_strings(),
            diffguard_domain::Language::Rust,
        );
        let result = p.sanitize_line(&line);
        prop_assert_eq!(result.len(), line.len());
    ...
    // Property: Output length is preserved (no masks)
    #[test]
    fn property_sanitize_line_length_preserved_no_masks(
        line in prop::string::string_regex("[ -~]{0,100}").unwrap(),
    ) {
        let mut p = diffguard_domain::Preprocessor::with_language(
            diffguard_domain::PreprocessOptions::none(),
            diffguard_domain::Language::Rust,
        );
        let result = p.sanitize_line(&line);
        prop_assert_eq!(result.len(), line.len());
    ...
    // Property: No masking when disabled produces unchanged output
    #[test]
    fn property_sanitize_line_no_masking_unchanged(
        line in prop::string::string_regex("[ -~]{0,100}").unwrap(),
    ) {
        let mut p = diffguard_domain::Preprocessor::with_language(
            diffguard_domain::PreprocessOptions::none(),
    ...
    // Property: All masked positions contain spaces or original character
    #[test]
    fn property_sanitize_line_masked_are_spaces_or_original(
        line in prop::string::string_regex("[ -~]{0,100}").unwrap(),
    ) {
        ...
        prop_assert_eq!(input.len(), result_bytes.len());

        for (in_byte, out_byte) in input.iter().zip(result_bytes.iter()) {
            prop_assert!(*out_byte == *in_byte || *out_byte == b' ');
        }
    }

    // Property: Empty line preserves length
    #[test]
    fn property_sanitize_line_empty_line_length() {
        let line = "";
        let mut p = diffguard_domain::Preprocessor::with_language(
            diffguard_domain::PreprocessOptions::comments_and_strings(),
            diffguard_domain::Language::Rust,
    ...
    // Property: Various languages preserve length
    #[test]
    fn property_sanitize_line_length_various_languages(
        line in prop::string::string_regex("[ -~]{0,100}").unwrap(),
        lang in prop::sample::select(vec![
            diffguard_domain::Language::Rust,
            diffguard_domain::Language::Python,
            diffguard_domain::Language::JavaScript,
            diffguard_domain::Language::Go,
            diffguard_domain::Language::C,
            diffguard_domain::Language::Cpp,
            diffguard_domain::Language::Java,
            diffguard_domain::Language::Ruby,
            diffguard_domain::Language::Shell,
        ]),
    ) {
        let mut p = diffguard_domain::Preprocessor::with_language(
            diffguard_domain::PreprocessOptions::comments_and_strings(),
