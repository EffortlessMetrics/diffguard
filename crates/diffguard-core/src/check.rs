...
pub fn collect_blame_allowed_lines(
    ...
    for (path, lines) in lines_by_path {
        let Some(blame_text) = blame_by_path.get(&path) else {
            continue;
        };
...
/// Collect all candidate override file paths for files in the diff.
pub fn collect_override_candidates_for_diff(
    diff_text: &str,
    scope: diffguard_types::Scope,
...
pub fn load_directory_overrides_for_diff(
    ...
    for override_path in candidate_paths {
        let Some(contents) = override_contents.get(&override_path) else {
            continue;
        };

        let parsed: DirectoryOverrideConfig = toml::from_str(contents).with_context(|| {
            format!(
