...
/// Get the presets.rs source content using CARGO_MANIFEST_DIR.
/// For tests in crates/diffguard/tests/, CARGO_MANIFEST_DIR = crates/diffguard
fn get_presets_source() -> String {
    fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/src/presets.rs"))
        .expect("presets.rs source file should exist at crates/diffguard/src/presets.rs")
}
