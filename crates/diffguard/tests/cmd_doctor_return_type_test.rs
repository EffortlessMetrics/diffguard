...
fn test_cmd_doctor_no_unnecessary_wraps_warning() {
    ...
    assert!(
        !stderr.to_lowercase().contains("cmd_doctor") || !stderr.contains("unnecessary"),
...
fn test_cmd_doctor_return_type_is_i32() {
    ...
    // Run clippy to check for unnecessary_wraps - should fail if signature is still Result<i32>
    let output = StdCommand::new("cargo")
        .args([
            "clippy",
            "-p",
    ...
    assert!(
        !stderr.contains("unnecessary_wraps") && !stderr.contains("cmd_doctor"),
