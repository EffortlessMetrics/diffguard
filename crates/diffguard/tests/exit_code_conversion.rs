...
//!
//! Since main() is decorated with #[cfg(not(test))], these tests work by:
//! 1. Running the CLI with inputs designed to produce specific exit codes
//! 2. Verifying the actual process exit code matches expectations
//!
//! The ACTUAL conversion logic is tested via a hypothetical extracted helper
//! that will be created during the fix. These tests will compile and pass
//! when the TryFrom-based conversion is implemented.
...
fn test_exit_code_3_warn_fail_preserved() {
    ...
    // Create a warning-level finding (long line without panic)
    // Actually, let's use a different approach - write a config that triggers warn
    let config = r#"
[[rules]]
id = "long-line"
pattern = "............."
...
"#;
    ...
    // With --fail-on warn, warnings should produce exit code 3
    let mut cmd = diffguard_cmd();
    cmd.current_dir(dir)
        .arg("check")
        .arg("--base", &base)
