...
use diffguard_diff::{parse_unified_diff, DiffLine};
use diffguard_types::Scope;

// Import fixtures under test
use diffguard_bench::fixtures::{
    convert_diff_line_to_input_line, convert_diff_lines_to_input_lines,
    generate_input_lines, generate_lines_with_comment_density, generate_mixed_unified_diff,
...
fn test_generate_unified_diff_single_line() {
    ...
    assert_eq!(lines.len(), 6, "Single line diff should have exactly 6 lines total");
}
...
fn test_generate_unified_diff_large() {
    ...
    assert_eq!(lines.len(), 100_005, "100K line diff should have exactly 100005 lines");
}
...
fn test_generate_unified_diff_parsed_correctly() {
    ...
    let (diff_lines, stats) = parse_unified_diff(&diff_text, Scope::Added)
        .expect("Generated diff should be parseable");
...
fn test_generate_unified_diff_parsed_with_scope_added() {
    ...
    let (diff_lines, _) = parse_unified_diff(&diff_text, Scope::Added)
        .expect("Should parse with Added scope");
...
fn test_generate_mixed_unified_diff_produces_all_change_types() {
    ...
    let (diff_lines, _) = parse_unified_diff(&diff_text, Scope::Added)
        .expect("Mixed diff should be parseable");
    
    // With 30 lines and modulo 3, we expect ~10 added lines (when i % 3 == 0)
    assert!(diff_lines.len() >= 10, "Should have added lines in mixed diff");
}
...
fn test_generate_mixed_unified_diff_multiple_hunks() {
    ...
    assert!(hunk_count >= 10, "100 lines with chunk_size=10 should produce multiple hunks");
}
...
fn test_generate_lines_zero_density() {
    ...
    for line in &lines {
        assert!(!line.starts_with("// "), "Zero density should produce no comments: {}", line);
        assert!(!line.starts_with("/* "), "Zero density should produce no block comments: {}", line);
    }
}
...
fn test_generate_lines_full_density() {
    ...
    let comment_count = lines.iter().filter(|l| l.starts_with("// ") || l.starts_with("/* ")).count();
...
fn test_generate_lines_partial_density() {
    ...
    for &density in &[0.25, 0.50, 0.75] {
        ...
        let comment_count = lines.iter()
            .filter(|l| l.starts_with("// ") || l.starts_with("/* ") || l.contains(" block comment "))
            .count();
        let actual_density = comment_count as f32 / 1000.0;
        // Allow 10% tolerance
        assert!((actual_density - density).abs() < 0.10, 
            "Density {} produced {}% comments, expected ~{}%", 
            density, actual_density * 100.0, density * 100.0);
    }
}
...
fn test_generate_lines_preserves_line_count() {
    for num_lines in [0, 1, 10, 100, 1000] {
        ...
        assert_eq!(lines.len(), num_lines, "Line count should match for num_lines={}", num_lines);
    }
}
...
fn test_preprocessor_different_languages() {
    for lang in [Language::Rust, Language::Python, Language::JavaScript, Language::Go] {
        ...
        let result = preprocessor.sanitize_line("let x = 1;");
        assert_eq!(result.len(), "let x = 1;".len(), 
            "Preprocessor for {:?} should handle plain code", lang);
    }
}
...
fn test_full_pipeline_generate_diff_parse_convert_evaluate() {
    ...
    // Parse it
    let (diff_lines, _) = parse_unified_diff(&diff_text, Scope::Added)
        .expect("Should parse generated diff");
    ...
    assert!(!evaluation.findings.is_empty(), "Should find 'line' in generated content");
}
